...
describe('#handleBalanceUpdate', () => {
  ...
  describe('updating tokens', () => {
    it('should add all tokens to the known tokens', () => {
      const address = randomStr()
    ...
    it('should attempt to remove any zero balance tokens', () => {
      const address = randomStr()
      const balance = Balance(1, '0xaddress', '0x00')
      getCustomTokens.mockReturnValueOnce([])
      handleBalanceUpdate(address, [balance], [balance.chainId], 'snapshot')
      expect(storeApi.removeKnownTokens).toHaveBeenCalledWith(address, new Set(['1:0xaddress']))
    })
  })
})
