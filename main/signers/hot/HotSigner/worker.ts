...
function sendToMainProcess(message: WorkerTokenMessage | WorkerRPCMessage) {
  if (process.send) {
...
export class HotSignerWorkerController {
  ...
  constructor(type: HotSignerType) {
    if (type === 'ring') {
      this.worker = new RingSignerWorker()
    } else if (type === 'seed') {
      this.worker = new SeedSignerWorker()
    } else {
      process.exit(2)
    }

    process.on('message', (message: RPCMessage) => this.handleMessage(message))
