...
export class Provider extends EventEmitter {
  ...
  send (payload: RPCRequestPayload, res: RPCRequestCallback = () => {}) {
    ...
    const signTypedDataMatcher = /eth_signTypedData_?(_v[134]$|$)/
    const signTypedDataRequest = method.match(signTypedDataMatcher)

    if (signTypedDataRequest) {
      const versionGroup = signTypedDataRequest[1] || 'v1'
      const version = versionGroup.substring(versionGroup.indexOf('_') + 1).toUpperCase() as Version
