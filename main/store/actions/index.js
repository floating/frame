...
module.exports = {
  ...
  toggleNonceAdjust: (u) =>
    u("main.nonceAdjust", (nonceAdjust) => !nonceAdjust),
  toggleShowLocalNameWithENS: (u) =>
    u(
      "main.showLocalNameWithENS",
      (showLocalNameWithENS) => !showLocalNameWithENS
    ),
    ...
    try {
      ...
      u("main", (main) => {
        ...
        main.networksMeta[updatedNetwork.type][updatedNetwork.id] =
          main.networksMeta[updatedNetwork.type][updatedNetwork.id] || {};
        main.networksMeta[updatedNetwork.type][updatedNetwork.id].symbol =
          newNet.symbol;

        main.networksMeta[updatedNetwork.type][
          updatedNetwork.id
        ].nativeCurrency =
          main.networksMeta[updatedNetwork.type][updatedNetwork.id]
            .nativeCurrency || {};
        main.networksMeta[updatedNetwork.type][
          updatedNetwork.id
        ].nativeCurrency.symbol = newNet.symbol;
  ...
  setBlockHeight: (u, chainId, blockHeight) => {
    u("main.networksMeta.ethereum", (chainsMeta) => {
      if (chainsMeta[chainId]) {
        ...
        log.error(
          `Action Error: setBlockHeight chainId: ${chainId} not found in chainsMeta`
        );
      }
      return chainsMeta;
    });
  ...
  setChainColor: (u, chainId, color) => {
    u("main.networksMeta.ethereum", (chainsMeta) => {
      if (chainsMeta[chainId]) {
        ...
        log.error(
          `Action Error: setChainColor chainId: ${chainId} not found in chainsMeta`
        );
      }
      return chainsMeta;
    });
    ...
    u("main.balances", address, (balances = []) => {
      const existingBalances = balances.filter((b) => {
        return newBalances.every(
          (bal) => bal.chainId !== b.chainId || bal.address !== b.address
        );
      });
    ...
    u("main.balances", (balances = {}) => {
      ...
      for (const accountAddress in balances) {
        const balanceIndex = balances[accountAddress].findIndex(
          (balance) =>
            balance.chainId === chainId && balance.address.toLowerCase() === key
        );
  ...
  addCustomTokens: (u, tokens) => {
    u("main.tokens.custom", (existing) => {
      ...
      const tokensToAdd = tokens.map((t) => ({
        ...t,
