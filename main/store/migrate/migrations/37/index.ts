...
const ParsedChainSchema = z.union([v37ChainSchema, z.boolean()]).catch(false)
...
const EthereumChainsSchema = z.record(z.coerce.number(), ParsedChainSchema).transform((chains) => {
  ...
  return Object.fromEntries(
    Object.entries(chains).filter(([id, chain]) => {
      if (chain === false) {
        log.info(`Migration 37: removing invalid chain ${id} from state`)
        return false
      }
...
const ChainsSchema = v37ChainsSchema.merge(
  z.object({
    ethereum: EthereumChainsSchema
  })
)

const MainSchema = v37MainSchema
  .merge(
    z.object({
...
const StateSchema = v37StateSchema.merge(z.object({ main: MainSchema })).passthrough()
...
const migrate = (initial: unknown) => {
  ...
  const updateChain = (chain: v37Chain) => {
    const removeRpcConnection = (connection: v37Connection) => {
      ...
      if (isServiceRpc) {
        log.info(`Migration 37: removing ${connection.current} preset from chain ${chain.id}`)
  ...
  try {
    ...
    const migratedChains = chainEntries
      ...
      .map(([id, chain]) => [id, updateChain(chain as v37Chain)])
