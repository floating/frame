...
const londonHardforkSigners: SignerCompatibilityByVersion = {
  ...
  ring: () => true,
  ledger: (version) =>
    version.major >= 2 || (version.major >= 1 && version.minor >= 9),
  trezor: (version, model) => {
    if ((model || '').toLowerCase() === 'trezor one') {
      return (
        version.major >= 2 ||
        (version.major >= 1 &&
          (version.minor > 10 || (version.minor === 10 && version.patch >= 4)))
      );
    }
...
function maxFee(rawTx: TransactionData) {
  ...
  if (
    [1, 3, 4, 5, 6, 10, 42, 61, 62, 63, 69, 42161, 421611].includes(chainId)
  ) {
    return 2 * 1e18;
  }
...
function populate(
  ...
  const useFrameMaxFeePerGas =
    !rawTx.maxFeePerGas || isNaN(parseInt(rawTx.maxFeePerGas, 16));
  const useFrameMaxPriorityFeePerGas =
    !rawTx.maxPriorityFeePerGas ||
    isNaN(parseInt(rawTx.maxPriorityFeePerGas, 16));
...
function serializeSignature({ v, r, s }: Signature) {
...
async function sign(
  rawTx: TransactionData,
  signingFn: (tx: TransactionData) => Promise<Signature>
): Promise<string> {
  return signingFn(rawTx).then((sig) => {
    const signature = serializeSignature(sig);
    const unsignedTx = convertToUnsignedTransaction(rawTx)
    return serializeTransaction(unsignedTx, signature)
  });
}
