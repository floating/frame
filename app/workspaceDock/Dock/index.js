...
const DockIcon = styled.div`
  ...
  /* transition: ${({ animate }) => (animate ? 'all 0.1s linear' : 'none')}; */
...
const DockCard = styled.div`
  ...
  /* transform: ${({ hide }) => (hide ? 'translateY(32px)' : 'translateY(-8px)')}; */
...
const Dock = () => {
  ...
  const handleMouseMove = (e) => {
    ...
    //   const iconWidth = 42 // Original width of the icon
    //   totalWidth += iconWidth * scale
    // })

    // // Adding margins after calculating scaled widths
    // const margin = 16 // Total margin for each icon (8 pixels on each side)
    // totalWidth += margin * iconsRef.current.length
  ...
  const calculateStyling = (iconIndex) => {
    ...
    if (distance < threshold) {
      ...
      console.log('(growth / mouseXFadeZone)', growth / mouseXFadeZone)
      console.log('mouseY', mouseY)
      console.log('Math.min(mouseY / mouseXFadeZone, 1)', Math.min(mouseY / mouseXFadeZone, 1))
      const adjustedGrowth = growth * Math.min(mouseY / mouseXFadeZone, 1)
      console.log('adjustedGrowth', adjustedGrowth)
      const scale = 1 + adjustedGrowth
      const shadow = baseShadow * scale * scale * scale * scale
      const borderRadius = baseBorderRadius * scale
      // const contentScale = 1 + (0.5 * (threshold - distance)) / threshold // Adjust this value as per the desired scaling effect on content
...
//   /* The mask */
//   -webkit-mask-box-image: radial-gradient(circle at top right, transparent 0% 8px, black 8px);
//   mask-box-image: radial-gradient(circle at top right, transparent 0% 8px, black 8px);
// `
...
//   /* The mask */
//   -webkit-mask-box-image: radial-gradient(circle at top left, transparent 0% 8px, black 8px);
//   mask-box-image: radial-gradient(circle at top left, transparent 0% 8px, black 8px);
// `
