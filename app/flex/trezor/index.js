...
const { default: TrezorConnect, DEVICE_EVENT, DEVICE, UI_EVENT, UI } = require('trezor-connect')
...
class Device {
  ...
  getPublicKey(path, cb) {
    TrezorConnect.getPublicKey({ device: this.device, path })
      .then((res) => {
  ...
  ethereumGetAddress(path, showOnTrezor = false, cb) {
    TrezorConnect.ethereumGetAddress({ device: this.device, path, showOnTrezor })
      .then((res) => {
        if (!res.success) return cb(new Error(res.payload.error))
        cb(null, res.payload)
      })
  ...
  ethereumSignTransaction(path, transaction, cb) {
    TrezorConnect.ethereumSignTransaction({ device: this.device, path, transaction })
      .then((res) => {
        if (!res.success) return cb(new Error(res.payload.error))
        cb(null, res.payload)
      })
  ...
  ethereumSignMessage(path, message, cb) {
    TrezorConnect.ethereumSignMessage({ device: this.device, path, message, hex: true })
      .then((res) => {
        if (!res.success) return cb(new Error(res.payload.error))
        cb(null, res.payload)
      })
  ...
  ethereumVerifyMessage(path, address, message, signature, cb) {
    TrezorConnect.ethereumVerifyMessage({ device: this.device, path, address, message, signature })
      .then((res) => {
        if (!res.success) return cb(new Error(res.payload.error))
        cb(null, res.payload)
      })
