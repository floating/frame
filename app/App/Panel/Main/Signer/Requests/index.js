...
class Requests extends React.Component {
  ...
  render () {
    ...
    let normal = requests.filter(req => {
      return req.mode === 'normal'
    }).sort((a, b) => {
      if (a.created > b.created) return -1
      if (a.created < b.created) return 1
      return 0
    })
    let monitor = requests.filter(req => {
      return req.mode === 'monitor'
    }).sort((a, b) => {
      if (a.created > b.created) return -1
      if (a.created < b.created) return 1
      return 0
    ...
    normal = normal.map((req, index) => {
      return {req, bottom: containMonitor + (index * 10)}
    })
    monitor = monitor.map((req, index) => {
      return {req, bottom: ((monitor.length - 1 - index) * 90)}
    })
      ...
      <div className={this.store('signer.view') === 'default' ? 'signerRequests' : 'signerRequests signerRequestsHidden'}>
        ...
        <div className='requestContainerWrap'>
          <CSSTransitionGroup className='requestContainer' style={{ height: containHeight + 'px' }} transitionName='slideUp' transitionEnterTimeout={960} transitionLeaveTimeout={640}>
            ...
            {normal.concat(monitor).map((item, i) => {
              let { req, bottom } = item
